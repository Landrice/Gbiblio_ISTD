...
//        int x = (args.length > 0) ? 1 : 2;
        ...
        con = dbCon.geConnection();
        try {
            pst = con.prepareStatement("UPDATE "+db+".droitutilisateur SET ajoutDocument=?,ajoutAuteur=?,ajoutCategorie=?,ajoutEditeur=?,ajoutMembre=?,miseAjourDocument=?,miseAjourAuteur=?,miseAjourCategorie=?,miseAjourEditeur=?,miseAjourMembre=?,gererUtilisateur=? WHERE idUtilisateur=?");
//            pst.setString(1, id);
